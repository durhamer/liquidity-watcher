...
def get_macro_data(api_key, days):
    ...
    try:
        ...
        df = pd.DataFrame({
            ...
            'Bank_Credit': bank_credit, 'Delinq_Consumer': delinq_consumer,
            'Delinq_Corp': delinq_corp, 'HY_Spread': hy_spread
...
if api_key_input:
    ...
    if df_macro is not None:
        ...
        if stock_series is not None:
            ...
            display_df = merged_df[merged_df.index >= f"{display_start_year}-01-01"]

            tab1, tab2, tab3, tab4, tab5 = st.tabs(["💧 流動性估值", "📉 殖利率曲線", "☢️ VPIN", "🏦 違約監控", "🧮 相關性"])
